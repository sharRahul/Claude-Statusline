...
A shell script that adds a live status bar to [Claude Code](https://claude.ai/code), showing your active model, effort level, context window usage, Claude.ai rolling usage percentages, session ID, and current folder — updated automatically after every message.

```
Sonnet 4.6 [Low] | ████░░░░░░ 37% 74K/200K | 5h:12% ↺ 2h 14m | 7d:45% ↺ 3d 6h | abc12345 | my-project
```
...
1. Parses the transcript file at `transcript_path` to count used tokens.
...
transcript file  ──(token counts)───────────────────>  statusline.sh
...
| **Context bar** | `████░░░░░░` | Visual fill of context window consumed; color turns yellow ≥60%, red ≥80% |
...
| **Session ID** | `abc12345` | First 8 characters of the current session UUID |
...
On Windows, Claude Code's `statusLine` command runs via **Git Bash** (the `sh` that ships with [Git for Windows](https://git-scm.com/download/win)). All POSIX tools the script needs (`awk`, `date`, `stat`, `basename`) are included in Git Bash.

### 4. refresh_usage.sh (for 5h/7d data)
